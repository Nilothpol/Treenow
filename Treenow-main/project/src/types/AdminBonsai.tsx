...
const AdminBonsai = () => {
  ...
  const fetchBonsai = async () => {
    ...
    try {
      const { data } = await axios.get('http://localhost:5000/api/bonsai', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post('http://localhost:5000/api/bonsai', formData, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/bonsai/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
