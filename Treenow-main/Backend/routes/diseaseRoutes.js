...
// ✅ GET all diseases (Public or Protected)
router.get('/', async (req, res) => {
  try {
    const diseases = await Disease.find().populate('tree').populate('products');
    res.json(diseases);
  } catch (error) {
...
// ✅ GET single disease by ID (Public or Protected)
...
router.post('/', protect, admin, async (req, res) => {
  try {
    ...
    const newDisease = new Disease({
      ...
      symptoms,
      tree,
      products: products || [], // Optional products
    });
